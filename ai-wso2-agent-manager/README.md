...
Download `values/observability-extension.yaml` and set the OpenSearch credentials:

```bash
curl -sOL ${MODULE_RAW}/values/observability-extension.yaml
# Edit observability-extension.yaml to set opensearch.username and opensearch.password, then:
...
Set `REGISTRY_ENDPOINT` to the container registry endpoint configured in the OpenChoreo workflow plane, then install:

```bash
export REGISTRY_ENDPOINT="<registry-endpoint>"
